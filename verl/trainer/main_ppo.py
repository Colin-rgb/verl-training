...
class RewardManager():
    ...
    def __init__(self, tokenizer, num_examine,config) -> None:
        ...
        if verifier_name == 'prime': # TODO: support default verifiers
            ...
            # 如果没有找到对应的验证器，可以引发一个更明确的错误
            raise NotImplementedError(f"Verifier '{verifier_name}' is not supported.")
    ...
    # ==================== MODIFICATION START: RewardManager __call__ 方法重构 ====================
    def __call__(self, data: DataProto):
        """
        重构此方法以清晰地处理多种奖励来源，特别是我们新增的 'em_rl_sequence'。
        """
        reward_tensor_dict = {}
        reward_metrics = {}
        reward_tensor = torch.zeros_like(data.batch['responses'], dtype=torch.float32)

        # 步骤 1：处理来自验证器的奖励（如果启用）
        ...
        if 'rm_scores' in data.batch.keys() and self.config.reward_model.rm_coef != 0:
            ...
            if rm_type == 'em_rl_sequence':
                print("DEBUG: RewardManager is processing 'em_rl_sequence' reward.")
            ...
            # 分支 2.2：为未来的 token 级奖励预留位置
            elif rm_type == 'em_rl_token':
                raise NotImplementedError("'em_rl_token' reward processing is not yet implemented in RewardManager.")
            
            # 分支 2.3：处理框架原有的标准奖励模型
            elif rm_type in ['normal', 'prime']:
                print("DEBUG: RewardManager is processing standard model-based reward.")
                ...
                warnings.warn(f"Warning: 'rm_scores' present but rm_type '{rm_type}' is not explicitly handled. Treating as a generic reward.")
    ...
    # ===================== MODIFICATION END: RewardManager __call__ 方法重构 =====================
...
def main_task(config):
    ...
    # ==================== MODIFICATION START: main_task Worker 选择 ====================
    # 重构此部分以支持 'em_rl_sequence' 及其 FSDP 后端
    if config.reward_model.enable and config.reward_model.rm_coef != 0.:
        ...
        # 分支 3：处理我们新增的序列熵奖励
        elif rm_type == 'em_rl_sequence':
            print("INFO: Configuring EM-RL Sequence Entropy Reward in main_task.")
            if config.reward_model.strategy == 'fsdp':
                from verl.workers.fsdp_workers import FSDPSequenceEntropyRewardWorker
                role_worker_mapping[Role.RewardModel] = ray.remote(FSDPSequenceEntropyRewardWorker)
            elif config.reward_model.strategy == 'megatron':
                from verl.workers.megatron_workers import MegatronSequenceEntropyRewardWorker
                role_worker_mapping[Role.RewardModel] = ray.remote(MegatronSequenceEntropyRewardWorker)
            else:
                raise NotImplementedError(f"Strategy '{config.reward_model.strategy}' not supported for EM-RL Sequence.")
        
        else:
            raise NotImplementedError(f"Unknown reward model type: {rm_type}")
        
        mapping[Role.RewardModel] = global_pool_id
    # ===================== MODIFICATION END: main_task Worker 选择 =====================
