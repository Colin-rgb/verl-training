...
from .reward_model.megatron.sequence_entropy_reward_model import MegatronSequenceEntropyRewardModel
...
class MegatronSequenceEntropyRewardWorker(RewardModelWorker):
    """
    一个专门用于序列熵奖励的 Worker。
    它会实例化 MegatronSequenceEntropyRewardModel。
    """
    @register(dispatch_mode=Dispatch.ONE_TO_ALL)
    def init_model(self):
        # 重写 init_model 方法
        ...
        megatron_config = OmegaConf.create({
            'sequence_parallel': self.config.megatron.get('sequence_parallel', True),
            'param_dtype': PrecisionType.to_str(torch.bfloat16), # 与主模型保持一致
            'tensor_model_parallel_size': mpu.get_tensor_model_parallel_world_size(),
            'pipeline_model_parallel_rank': mpu.get_pipeline_model_parallel_rank(),
            'pipeline_model_parallel_size': mpu.get_pipeline_model_parallel_world_size(),
            'virtual_pipeline_model_parallel_rank': mpu.get_virtual_pipeline_model_parallel_rank(),
        ...
        init_model_parallel_config(megatron_config)

        # 实例化我们新的序列熵奖励模型
        self.rm = MegatronSequenceEntropyRewardModel(config=self.config,
                                                     model_config=None,
                                                     reward_model_module=None,
                                                     megatron_config=megatron_config)
